...
6. **Comment the landing on the issue**: the sha and anything a reviewer needs to know. No screenshots: `gh` cannot attach an image to a comment, and the visual result is checked by the human on the epic PR. Done when: the comment is posted.
