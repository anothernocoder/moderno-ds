...
- `@moderno-ui/tokens` package: shadcn-style semantic slot contract, neutral OKLCH defaults (light `:root` + dark `.dark`), Tailwind v4 preset (`@theme inline`), CSS vars output. **No brand identity.**
- `@moderno-ui/css` package: thin entrypoint that re-exports `@moderno-ui/tokens/css` (+ placeholder for `@moderno-ui/core/css` arriving in Phase 1) via `package.json` `exports`.
...
| F0.2 | `@moderno-ui/tokens` exports CSS vars for the **entire** minimum contract: `--background`, `--foreground`, `--card(-foreground)`, `--popover(-foreground)`, `--primary(-foreground)`, `--secondary(-foreground)`, `--muted(-foreground)`, `--accent(-foreground)`, `--destructive(-foreground)`, `--border`, `--input`, `--ring`, `--radius`, `--font-sans`, `--font-mono`, `--chart-1…5`. |
