...
Establish the monorepo skeleton and the single source of truth for styling: DTCG tokens in OKLCH → CSS vars + Tailwind v4 preset, with a public CSS entrypoint (`@moderno-ui/css`) and the technical contract written in `CONTRACT.md`. Everything else depends on this layer.
...
- `@moderno-ui/css` package: shadcn-style semantic slot contract, neutral OKLCH defaults (light `:root` + dark `.dark`), Tailwind v4 preset (`@theme inline`), CSS vars output, and the single public entrypoint (+ placeholder for `@moderno-ui/core/css` arriving in Phase 1) via `package.json` `exports`. **No brand identity.** (Phase 0 shipped the contract as a separate tokens package; ADR-0008 merged it here.)
...
| F0.2 | `@moderno-ui/css` exports CSS vars for the **entire** minimum contract: `--background`, `--foreground`, `--card(-foreground)`, `--popover(-foreground)`, `--primary(-foreground)`, `--secondary(-foreground)`, `--muted(-foreground)`, `--accent(-foreground)`, `--destructive(-foreground)`, `--border`, `--input`, `--ring`, `--radius`, `--font-sans`, `--font-mono`, `--chart-1…5`. |
...
| F0.6 | `@moderno-ui/css` importable as `@import "@moderno-ui/css"` without exposing internal paths (`dist/`, subpaths).                                                                                                                                                                                                                                                                        |
...
- `packages/css/` publishable (CSS-first, no JS bundler).
...
- [ ] A test HTML importing only `@moderno-ui/css` renders with the neutral defaults and responds to `.dark`.
