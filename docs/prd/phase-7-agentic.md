...
- [`phase-7/validate-rules.md`](phase-7/validate-rules.md) — the starter deterministic rule set behind `validate_usage` and `@moderno-ui/lint`.
...
- **`@moderno-ui/mcp`** — local stdio MCP server. Five tools, all framework-parameterized
...
- **Discovery** — `@moderno-ui/cli init` writes an `AGENTS.md` stanza (golden rules +
...
- `packages/mcp/` — `@moderno-ui/mcp`, publishable, runnable via npx/bunx.
- `packages/lint/` — `@moderno-ui/lint` (ESLint plugin + CLI) over the shared rule engine.
...
- [ ] An agent in a sample repo installs Moderno, configures `@moderno-ui/mcp`, and builds a screen using only server-retrieved APIs — no hallucinated props.
- [ ] `get_component_api` for a repo pinned to an older `@moderno-ui/react` returns that version's API, not latest.
- [ ] `validate_usage` catches a hardcoded color, an invalid prop, and a raw-Ark import in a sample snippet; `@moderno-ui/lint` reports the same three.
...
- Phase 5 (`registry.json`, `@moderno-ui/cli`) — `init` stanza + CLI action plane.
...
- Write tools in the MCP (scaffold/add/init) — mutations stay with `@moderno-ui/cli`.
