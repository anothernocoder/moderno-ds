...
**DESIGN.md**:
The guide to Moderno's **default theme** (`theme-moderno`), in the google-labs `DESIGN.md` format: YAML front matter plus rationale. Its front matter is generated from `registry/themes/theme-moderno/tokens.dtcg.json`, the one hand-edited source of the theme's values; its body explains them without restating them (ADR-0007). `DESIGN.md` + that `tokens.dtcg.json` supply _values_; `CONTRACT.md` supplies _names and rules_.
...
**Theme item**:
A registry unit that delivers brand CSS. It names its faces in `--font-sans`/`--font-serif` but ships no font files: the consumer loads them (see the docs installation page). Installable via the CLI (`moderno add theme-moderno`). Authoring: `tokens.dtcg.json` + `theme.css` in `registry/themes/{name}/`, generated by the **Theme Builder** or `pnpm theme:build` (`tooling/theme-compile`). `tokens.dtcg.json` is the hand-edited source for every theme, `theme-moderno` included; `theme.css` is compiled from it. Not an npm package.
