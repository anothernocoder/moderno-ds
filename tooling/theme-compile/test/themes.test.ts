...
/** The theme the root tokens.json authors (its one source, per DESIGN.md). */
const rootTokens = readFileSync(`${repoRoot}tokens.json`, "utf8");
const ROOT_THEME: string = JSON.parse(rootTokens).$extensions["style.moderno.theme"].name;

describe("the default theme has one source: the root tokens.json", () => {
  it("names a registry theme", () => {
    expect(themeNames).toContain(ROOT_THEME);
  });

  it("the registry copy is byte-for-byte the root tokens.json", () => {
    const copy = readFileSync(`${themesRoot}/${ROOT_THEME}/tokens.dtcg.json`, "utf8");
    expect(copy, "run `pnpm theme:build` after editing tokens.json").toBe(rootTokens);
  });

  it("DESIGN.md's front matter is exactly what tokens.json renders", () => {
    const markdown = readFileSync(`${repoRoot}DESIGN.md`, "utf8");
    const fresh = renderFrontMatter(
      JSON.parse(rootTokens),
      defaultsFrom(tokensCss),
      readMeta(markdown),
    );
    expect(
      withFrontMatter(markdown, fresh),
      "run `pnpm theme:build` after editing tokens.json",
    ).toBe(markdown);
  });
});
...
describe("registry themes compile and stay in sync", () => {
  ...
  it.each(themeNames)("%s: committed theme.css matches a fresh compile", (name) => {
    ...
    // The default theme is authored at the repo root; its banner names that file.
    const source = name === ROOT_THEME ? `${repoRoot}tokens.json` : input;
    const rel = source.replace(repoRoot, "");
