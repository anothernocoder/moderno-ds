/**
 * UI string tables. Page *content* lives in MDX (one file per locale); this
 * holds only chrome — nav, buttons, table headers, the Theme Builder labels.
 * Prop names and code identifiers are never translated (they are the real API);
 * only the surrounding labels are.
 */
export const locales = ["en", "es"] as const;
export type Locale = (typeof locales)[number];
export const defaultLocale: Locale = "en";

export function isLocale(value: string | undefined): value is Locale {
  return value !== undefined && (locales as readonly string[]).includes(value);
}

/** Resolve the active locale from Astro's i18n routing, with a safe fallback. */
export function currentLocale(value: string | undefined): Locale {
  return isLocale(value) ? value : defaultLocale;
}

/** `en/button` → `{ locale: "en", slug: "button" }`. */
export function splitId(id: string): { locale: Locale; slug: string } {
  const [first, ...rest] = id.split("/");
  const locale = first && isLocale(first) ? first : defaultLocale;
  return { locale, slug: rest.join("/") || "index" };
}

export const ui = {
  en: {
    "nav.docs": "Docs",
    "nav.components": "Components",
    "nav.themeBuilder": "Theme Builder",
    "nav.skipToContent": "Skip to content",
    "home.getStarted": "Get started",
    "sidebar.filterLabel": "Filter pages",
    "sidebar.filterPlaceholder": "Filter pages…",
    "sidebar.noMatches": "No matches",
    "search.label": "Search",
    "search.trigger": "Search docs…",
    "search.shortcut": "⌘K",
    "search.placeholder": "Search the docs…",
    "search.empty": "Type to search",
    "search.noResults": "No results",
    "search.indexMissing": "Search index not built yet. Run `pnpm build` to generate it.",
    "framework.label": "Framework",
    "siteTheme.label": "Site theme",
    "siteTheme.neutral": "Neutral",
    "copy.copied": "Copied",
    "copy.code": "Copy code",
    "copy.markdown": "Copy Markdown",
    "copy.viewMarkdown": "View .md",
    "code.expand": "Expand code",
    "code.collapse": "Collapse code",
    "install.manager": "Package manager",
    "nav.menu": "Menu",
    "pager.label": "Previous and next page",
    "pager.prev": "Previous",
    "pager.next": "Next",
    "props.name": "Prop",
    "props.type": "Type",
    "props.required": "required",
    "props.requiredLabel": "Required prop",
    "props.default": "Default",
    "props.description": "Description",
    "props.empty": "This component forwards the native element's attributes only.",
    "contract.slot": "Slot",
    "contract.step": "Step",
    "contract.value": "Default",
    "contract.size": "Size / line height",
    "contract.slotAndValue": "Slot and default",
    "contract.stepAndSize": "Step, size and line height",
    "contract.role": "Role",
    "install.title": "Installation",
    "toc.title": "On this page",
    "lang.switch": "Español",
    "lang.short": "ES",
    "theme.title": "Theme Builder",
    "theme.lead":
      "Edit the contract slots and brand primitives; the preview uses the real components.",
    "theme.light": "Light",
    "theme.dark": "Dark",
    "theme.name": "Name",
    "theme.startFrom": "Start from",
    "theme.import": "Import",
    "theme.paste": "Paste JSON",
    "theme.reset": "Reset",
    "theme.export": "Export",
    "theme.copy": "Copy",
    "theme.download": "Download",
    "theme.pickColor": "Pick a color",
    "theme.cliSnippet": "CLI",
    "theme.contrastOk": "WCAG AA",
    "theme.contrastFail": "Below AA",
    "theme.invalid": "Invalid theme",
    "theme.inherited": "inherits the default",
    "theme.group.surfaces": "Surfaces",
    "theme.group.brand": "Brand",
    "theme.group.support": "Support",
    "theme.group.charts": "Charts",
    "theme.group.other": "Other",
    "theme.group.extended": "Extended (optional)",
    "theme.customize": "Customize",
    "theme.closePanel": "Close panel",
    "theme.contrast": "Contrast",
    "theme.preview": "Preview",
    "theme.description": "Description",
    "theme.descriptionHint": "Optional: one sentence on the brand, for DESIGN.md",
    "theme.notesKept": "Brand notes from the base theme",
    "theme.notesDraft": "Brand notes drafted from the values",
    "theme.notesLoading": "Loading the brand notes…",
  },
  es: {
    "nav.docs": "Documentación",
    "nav.components": "Componentes",
    "nav.themeBuilder": "Editor de temas",
    "nav.skipToContent": "Saltar al contenido",
    "home.getStarted": "Empezar",
    "sidebar.filterLabel": "Filtrar páginas",
    "sidebar.filterPlaceholder": "Filtrar páginas…",
    "sidebar.noMatches": "Sin resultados",
    "search.label": "Buscar",
    "search.trigger": "Buscar…",
    "search.shortcut": "⌘K",
    "search.placeholder": "Buscar en la documentación…",
    "search.empty": "Escribe para buscar",
    "search.noResults": "Sin resultados",
    "search.indexMissing": "El índice de búsqueda aún no está generado. Ejecuta `pnpm build`.",
    "framework.label": "Framework",
    "siteTheme.label": "Tema del sitio",
    "siteTheme.neutral": "Neutro",
    "copy.copied": "Copiado",
    "copy.code": "Copiar código",
    "copy.markdown": "Copiar Markdown",
    "copy.viewMarkdown": "Ver .md",
    "code.expand": "Mostrar todo el código",
    "code.collapse": "Contraer código",
    "install.manager": "Gestor de paquetes",
    "nav.menu": "Menú",
    "pager.label": "Página anterior y siguiente",
    "pager.prev": "Anterior",
    "pager.next": "Siguiente",
    "props.name": "Propiedad",
    "props.type": "Tipo",
    "props.required": "obligatoria",
    "props.requiredLabel": "Propiedad obligatoria",
    "props.default": "Por defecto",
    "props.description": "Descripción",
    "props.empty": "Este componente solo reenvía los atributos del elemento nativo.",
    "contract.slot": "Slot",
    "contract.step": "Paso",
    "contract.value": "Por defecto",
    "contract.size": "Tamaño / interlineado",
    "contract.slotAndValue": "Slot y valor por defecto",
    "contract.stepAndSize": "Paso, tamaño e interlineado",
    "contract.role": "Rol",
    "install.title": "Instalación",
    "toc.title": "En esta página",
    "lang.switch": "English",
    "lang.short": "EN",
    "theme.title": "Editor de temas",
    "theme.lead":
      "Edita los slots del contrato y las primitivas de marca; la vista previa usa los componentes reales.",
    "theme.light": "Claro",
    "theme.dark": "Oscuro",
    "theme.name": "Nombre",
    "theme.startFrom": "Partir de",
    "theme.import": "Importar",
    "theme.paste": "Pegar JSON",
    "theme.reset": "Restablecer",
    "theme.export": "Exportar",
    "theme.copy": "Copiar",
    "theme.download": "Descargar",
    "theme.pickColor": "Elegir color",
    "theme.cliSnippet": "CLI",
    "theme.contrastOk": "WCAG AA",
    "theme.contrastFail": "Bajo AA",
    "theme.invalid": "Tema inválido",
    "theme.inherited": "hereda el valor por defecto",
    "theme.group.surfaces": "Superficies",
    "theme.group.brand": "Marca",
    "theme.group.support": "Soporte",
    "theme.group.charts": "Gráficas",
    "theme.group.other": "Otros",
    "theme.group.extended": "Extendidos (opcionales)",
    "theme.customize": "Personalizar",
    "theme.closePanel": "Cerrar panel",
    "theme.contrast": "Contraste",
    "theme.preview": "Vista previa",
    "theme.description": "Descripción",
    "theme.descriptionHint": "Opcional: una frase sobre la marca, para el DESIGN.md",
    "theme.notesKept": "Notas de marca del tema base",
    "theme.notesDraft": "Notas de marca en borrador, según los valores",
    "theme.notesLoading": "Cargando las notas de marca…",
  },
} as const;

export type UiKey = keyof (typeof ui)["en"];

export function useTranslations(locale: Locale) {
  return function t(key: UiKey): string {
    return ui[locale][key] ?? ui[defaultLocale][key];
  };
}
