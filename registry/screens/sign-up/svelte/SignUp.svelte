<!--
  SignUp — the full-viewport account-creation screen: the credential card in its
  sign-up mode (name, email, a new password, consent), and beside it the three
  things a person is actually deciding between when they weigh giving you an
  address. Copy it into your project with `moderno add sign-up-svelte`; the
  ...
  related fields with one actions row — and it says so itself: its guidance
  sends a sign-in or sign-up form here. A screen that mounted both would put two
  forms and two submits on one page.

  Presentational. The screen owns the viewport and nothing else: no credentials,
  no request, no router. It renders the `error` / `errors` / `loading` it is
  handed and reports what the reader did — `onsubmit` for the form, `onnavigate`
  for every link it draws itself. Whether the address is already taken, what a
  strong password is, and where a created account goes next are all the mounting
  page's business.
  ...
  the destination, so a client router can `preventDefault()` — and read `metaKey`
  before it does, to leave a ctrl/cmd-click to the browser — while the markup
  keeps its meaning without JavaScript.

  The highlights sit in an <aside> the screen paints itself, the way it paints
  its masthead and footer: they are the page's argument for signing up, not a
  reusable section, and the h2 inside names the landmark, so there is no
  `aria-label` repeating it and no `id` to collide when the screen is mounted
  more than once on a page (the docs mount it seven times).
  ...
  the wordmark shares a row with the sign-in link; at `@md` (--container-md,
  36rem) the footer stops stacking and the copyright shares a row with the legal
  links; at `@lg` (--container-lg, 48rem) the highlights leave their place under
  the card and stand beside it, so the form is at eye level and the reasons are
