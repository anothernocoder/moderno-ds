 ...
 * Signing up, only the field that was actually rejected is marked.
 */
const invalid = (field: string) =>
  signUp.value ? Boolean(props.errors?.[field]) : Boolean(props.error);
</script>
      ...
      <Card.Content>
        <form class="grid gap-5" novalidate @submit="emit('submit', $event)">
          ...
          <Field.Root required :invalid="invalid('email')" :disabled="inert">
            ...
            />
            <Field.ErrorText v-if="signUp">{{ errors?.email }}</Field.ErrorText>
          </Field.Root>
          ...
          <div v-else class="grid gap-3 @sm:flex @sm:items-center @sm:justify-between">
          ...
          <Button type="submit" class="w-full" :disabled="inert" :aria-busy="loading">
            ...
            <template v-else>{{ signUp ? "Create account" : "Sign in" }}</template>
          </Button>
        </form>
      </Card.Content>

      <Card.Footer class="justify-center">
        <p v-if="signUp" class="text-sm text-muted-foreground">
