<script lang="ts">
  ...
  // declares `@container`, and the plan grid goes one-up → three-up at `@md`,
  // the `--container-md` step of the token contract. Drop this block in a
  // sidebar and it stacks; drop it in a page and it spreads — with no viewport
  // media query, so it never has to know where it was mounted.
  import { Button } from "@moderno-ui/svelte";

  const plans = [
    { name: "Starter", price: "$0/mo", features: ["1 project", "Community support", "1 GB storage"] },
    { name: "Pro", price: "$29/mo", features: ["Unlimited projects", "Email support", "50 GB storage"] },
    { name: "Scale", price: "$99/mo", features: ["Unlimited projects", "Priority support", "1 TB storage"] },
  ];
</script>
