...
import Demo from "./fixtures/ToggleGroupFixture.svelte";

afterEach(cleanup);

describe("ToggleGroup surface (Svelte)", () => {
  it("exposes every Ark part, not a hand-maintained subset", async () => {
    const { ToggleGroup: ArkToggleGroup } = await import("@ark-ui/svelte");
...
describe("ToggleGroup", () => {
  it("applies the recipe to the root part, defaulting to ghost md", () => {
    render(Demo, { props: { variant: "outline" as const, size: "sm" as const } });
