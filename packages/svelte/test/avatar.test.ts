...
import Demo from "./fixtures/AvatarFixture.svelte";

afterEach(cleanup);

describe("Avatar surface (Svelte)", () => {
  it("exposes every Ark part, not a hand-maintained subset", async () => {
    const { Avatar: ArkAvatar } = await import("@ark-ui/svelte");
...
describe("Avatar (Svelte)", () => {
  it("applies the recipe to the root part, defaulting to an md circle", () => {
    render(Demo, { props: { size: "lg" as const, shape: "square" as const } });
