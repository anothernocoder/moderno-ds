...
import ToggleSection from "../../playground/sections/Toggle.svelte";

describe("Toggle SSR (Svelte, server-only island)", () => {
  it("server-renders its playground section to a stable HTML string", () => {
    const { html } = render(ToggleSection, { props: { open: false } });
    // Ark's toggle machine on a native button. The recipe lands on each root, the pressed state
    // reaches the server string (aria-pressed / data-state), and the Indicator renders its on or
