...
import ChartsSection from "../../playground/sections/Charts.svelte";

describe("Charts SSR (Svelte, server-only island)", () => {
  it("server-renders its playground section to a stable HTML string", () => {
    const { html } = render(ChartsSection, { props: { open: false } });
    // Every chart is drawn on the server: one sized, labelled-as-image SVG per chart type, with its
