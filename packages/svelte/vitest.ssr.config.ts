...
// The registry gate mounts screens that import their blocks through the `@/`
// path `moderno add` writes, so this project resolves that alias.
const manifest = fileURLToPath(new URL("../../registry/registry.json", import.meta.url));

export default defineConfig({
  plugins: [svelte()],
  resolve: { alias: registryBlockAliases(manifest, "svelte") },
  test: {
    name: "svelte-ssr",
    environment: "node",
