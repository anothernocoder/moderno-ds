<!--
  ...
  assistive tech; the children snippet becomes the label part. A bare dot named
  by aria-label takes role="img" (indicatorRole). The scope/part/variant attrs
  spread last so they can't be clobbered.
-->
<script lang="ts">
...
<span
  role={indicatorRole(Boolean(children), rest)}
  {...rest}
