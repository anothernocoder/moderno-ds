...
export interface RegistryFile {
  ...
  type: string;
  /** The owning item's `registry:*` type. */
  itemType: string;
 ...
 *
 * The exclusion is by *item*: a theme item also ships its DESIGN.md (a
 * `registry:file`), generated by the same `theme-compile` and prose, not code.
 */
...
export function registrySourceFiles(manifestPath: string): string[] {
  ...
  for (const file of registryFiles(manifestPath)) {
    if (UNLINTED_TYPES.has(file.type) || UNLINTED_TYPES.has(file.itemType)) continue;
    paths.add(file.path);
  }
  return [...paths].sort();
}
