...
describe("Pagination surface (Solid)", () => {
  it("exposes every Ark part, not a hand-maintained subset", async () => {
    const { Pagination: ArkPagination } = await import("@ark-ui/solid");
...
describe("Pagination", () => {
  ...
  it("follows a controlled page", async () => {
    ...
    render(() => <Demo page={page()} />);
    expect(current()?.textContent).toBe("2");
    setPage(7);
    await waitFor(() => expect(current()?.textContent).toBe("7"));
  });
});
