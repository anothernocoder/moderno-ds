...
import SwitchSection from "../../playground/sections/switch.jsx";

describe("Switch SSR (Solid)", () => {
  it("server-renders its playground section to a stable HTML string", () => {
    const html = renderToString(() => <SwitchSection open={false} />);
    // Ark's on/off machine. The on/off state reaches every part on the server,
    // the recipe lands on each root, and the hidden input already carries the
    // switch role (and its checked state) before hydration.
