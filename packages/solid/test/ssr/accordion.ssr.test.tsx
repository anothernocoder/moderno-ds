...
import AccordionSection from "../../playground/sections/accordion.jsx";

describe("Accordion SSR (Solid)", () => {
  it("server-renders its playground section to a stable HTML string", () => {
    const html = renderToString(() => <AccordionSection open={false} />);
    // Ark's accordion machine over collapsible items. The recipe lands on each
    // root, the open items reach the server string (aria-expanded +
    // data-state), every content is a region labelled by its own trigger's id,
    // the contents of closed items are hidden, and a disabled item's trigger is
    // a disabled button.
