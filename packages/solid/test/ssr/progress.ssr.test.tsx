...
import ProgressSection from "../../playground/sections/progress.jsx";

describe("Progress SSR (Solid)", () => {
  it("server-renders its playground section to a stable HTML string", () => {
    const html = renderToString(() => <ProgressSection open={false} />);
    // Ark's progress machine. The recipe lands on each root, the state reaches
    // the server, the track or the circle is the progressbar with its value,
    // and the percentage is inline on the range; an indeterminate progress has
    // no value and no width.
