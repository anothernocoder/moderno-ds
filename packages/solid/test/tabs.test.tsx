...
describe("Tabs surface (Solid)", () => {
  it("exposes every Ark part, not a hand-maintained subset", async () => {
    const { Tabs: ArkTabs } = await import("@ark-ui/solid");
...
function Demo(props: {
  ...
  return (
    ...
    >
      ...
      <For each={TABS}>
        {(tab) => <Tabs.Content value={tab.value}>{tab.label} panel</Tabs.Content>}
      </For>
    </Tabs.Root>
  );
}
...
describe("Tabs", () => {
  ...
  it("moves focus between tabs with the arrow keys, skipping a disabled tab", async () => {
    const user = userEvent.setup();
    render(() => <Demo defaultValue="password" />);
