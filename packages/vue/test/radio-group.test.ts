...
describe("RadioGroup surface (Vue)", () => {
  it("exposes every Ark part, not a hand-maintained subset", async () => {
    const { RadioGroup: ArkRadioGroup } = await import("@ark-ui/vue");
            ...
            h(
              ...
              () => [
                ...
                h(RadioGroup.ItemText, {}, () => [
                  option.label,
                  h(RadioGroup.ItemDescription, {}, () => option.description),
                ]),
...
describe("RadioGroup (Vue)", () => {
  ...
  it("lays out vertically by default and horizontally on request", () => {
    ...
    cleanup();
    render(Demo, { props: { orientation: "horizontal" } });
