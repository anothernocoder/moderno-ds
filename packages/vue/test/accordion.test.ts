...
import { Accordion, type AccordionSize, type AccordionVariant } from "../src/index.js";

afterEach(cleanup);

describe("Accordion surface (Vue)", () => {
  it("exposes every Ark part, not a hand-maintained subset", async () => {
    const { Accordion: ArkAccordion } = await import("@ark-ui/vue");
...
const Demo = defineComponent({
  props: {
    ...
    defaultValue: { type: Array as PropType<string[]>, default: () => ["shipping"] },
          ...
          ITEMS.map((item) =>
            h(
              ...
              () => [
                ...
                h(Accordion.ItemContent, {}, () => `${item.label} answer`),
              ],
            ),
          ),
      );
