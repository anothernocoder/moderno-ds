...
describe("Pagination surface (Vue)", () => {
  it("exposes every Ark part, not a hand-maintained subset", async () => {
    const { Pagination: ArkPagination } = await import("@ark-ui/vue");
...
const Demo = defineComponent({
  props: {
    ...
    boundaryCount: { type: Number, default: undefined },
    onPageChange: {
      type: Function as PropType<(details: PaginationPageChangeDetails) => void>,
      default: undefined,
    },
  },
  setup(props) {
...
describe("Pagination", () => {
  ...
  it("keeps one page at each end: Ark's Vue Pagination has no boundaryCount", () => {
    // React, Svelte and Solid show 1 2 … 19 20 here. Ark-Vue does not declare
    // the prop, so it falls through to the <nav> as an attribute.
    render(Demo, { props: { defaultPage: 10, count: 200, boundaryCount: 2 } });
    expect(row()).toEqual(["1", "…", "9", "10", "11", "…", "20"]);
    expect(part("root").getAttribute("boundarycount")).toBe("2");
  });
