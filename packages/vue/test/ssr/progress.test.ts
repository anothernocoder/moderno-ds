...
describe("Progress SSR (Vue)", () => {
  it("server-renders its playground section to a stable HTML string", async () => {
    const html = await renderSection(ProgressSection);
    // The recipe lands on each root, the state reaches the server, the track or
    // the circle is the progressbar with its value, and the percentage is
    // inline on the range; an indeterminate progress has no value and no width.
