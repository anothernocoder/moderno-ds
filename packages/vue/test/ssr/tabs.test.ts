...
describe("Tabs SSR (Vue)", () => {
  it("server-renders its playground section to a stable HTML string", async () => {
    const html = await renderSection(TabsSection);
    // The recipe lands on each root, the list is a tablist, the selected tab
    // reaches the server string (aria-selected + data-selected), every panel is
    // labelled by its own tab's id, the panels of unselected tabs are hidden,
    // and a disabled tab is a disabled button.
