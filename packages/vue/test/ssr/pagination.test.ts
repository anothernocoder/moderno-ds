...
import PaginationSection from "../../playground/sections/pagination.js";
import { renderSection } from "./render-section.js";

describe("Pagination SSR (Vue)", () => {
  it("server-renders its playground section to a stable HTML string", async () => {
    const html = await renderSection(PaginationSection);
    // The recipe lands on each root; each root reaches the server as a named
    // <nav> with its page list (an ellipsis on both sides of page 5), the
    // current page marked, and prev already disabled on the first page.
