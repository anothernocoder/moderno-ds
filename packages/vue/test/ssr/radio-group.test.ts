...
describe("RadioGroup SSR (Vue)", () => {
  it("server-renders its playground section to a stable HTML string", async () => {
    const html = await renderSection(RadioGroupSection);
    // The recipe and Ark's orientation land on each root, the checked item
    // reaches its parts on the server, the disabled group is marked, and every
    // native radio is already there with its checked / disabled state before
    // hydration.
