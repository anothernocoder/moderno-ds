...
import { Switch, type SwitchCheckedChangeDetails, type SwitchSize } from "../src/index.js";

afterEach(cleanup);

describe("Switch surface (React)", () => {
  it("exposes every Ark part, not a hand-maintained subset", async () => {
    const { Switch: ArkSwitch } = await import("@ark-ui/react");
...
describe("Switch", () => {
  it("applies the size recipe to the root part, defaulting to md", () => {
    ...
    expect(part("control")).toBeTruthy();
    expect(part("thumb")).toBeTruthy();
