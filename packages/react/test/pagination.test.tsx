...
import { Pagination, type PaginationSize, type PaginationPageChangeDetails } from "../src/index.js";

afterEach(cleanup);

describe("Pagination surface (React)", () => {
  it("exposes every Ark part, not a hand-maintained subset", async () => {
    const { Pagination: ArkPagination } = await import("@ark-ui/react");
...
describe("Pagination", () => {
  ...
  it("goes to a clicked page and reports it", async () => {
    ...
    await waitFor(() => expect(current()?.textContent).toBe("10"));
  ...
  it("steps with prev and next, and jumps with first and last", async () => {
    ...
    await waitFor(() => expect(current()?.textContent).toBe("6"));
    await user.click(part("prev-trigger"));
    await user.click(part("prev-trigger"));
    await waitFor(() => expect(current()?.textContent).toBe("4"));
    await user.click(part("last-trigger"));
    await waitFor(() => expect(current()?.textContent).toBe("10"));
    await user.click(part("first-trigger"));
    await waitFor(() => expect(current()?.textContent).toBe("1"));
  });
